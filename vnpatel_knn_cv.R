...
myknncv <- function(train, test, cl, k, numFolds)
{
   ...
   ###
   res <- train
   folds <- createFolds(train$Sepal.Length,k=numFolds)
   #print(train)
   #print(folds[1])
   #print(cl)
   ...
   for(i in 1:numFolds){
           ...
           ###
           #print(c(do.call("cbind",folds[i])))
           validation_set <- train[c(do.call("cbind",folds[i])), ]
           #print(validation_set)
           # rest of the folds as training
           ###
           cv_train <- train[-c(do.call("cbind",folds[i])), ]
           #print(cv_train)
           # train class variable
           ###
           cv_train_cl <- cl[-c(do.call("cbind",folds[i]))]
           #print(cv_train_cl[-c(do.call("cbind",folds[i]))])
           # predict using cv_train, cv_train_cl and validation_set
           ###
           knnpreds <- knn(cv_train,validation_set , cl = cv_train_cl, k = k , prob = TRUE)
           # calculate accuracy
